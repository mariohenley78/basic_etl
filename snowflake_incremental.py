...
def ejecutar_carga_incremental():
    ...
    try:
        # 1. Crear formato de archivo solo si no existe
        cursor.execute(f"""
            CREATE FILE FORMAT IF NOT EXISTS {FORMATO_CSV}
                TYPE = 'CSV'
                PARSE_HEADER = TRUE
        ...
        # 2. Crear el stage solo si no existe
        cursor.execute(f"CREATE STAGE IF NOT EXISTS {STAGE_TEMPORAL};")

        # 3. Subir el nuevo archivo al stage
        ...
        # 4. Insertar los nuevos registros en la tabla existente (COPY INTO)
        cursor.execute(f"""
        ...
        # 5. Limpiar el archivo procesado del Stage
        cursor.execute(f"REMOVE @{STAGE_TEMPORAL}/{nombre_archivo_gzip};")

        cursor.execute(f"SELECT COUNT(*) FROM {NOMBRE_TABLA};")
        total_registros = cursor.fetchone()[0]

        print(f"Carga incremental completada. Registros totales en '{NOMBRE_TABLA}': {total_registros}")
